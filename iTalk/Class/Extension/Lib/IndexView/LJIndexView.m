...
- (void)setDataSource:(id<LJIndexViewDataSource>)dataSource
{
    _dataSource = dataSource;
    self.indexItems = [dataSource sectionIndexTitlesForLJIndexView:self];
}
...
- (void)refreshIndexItems
{
    ...
    self.indexItems = [self.dataSource sectionIndexTitlesForLJIndexView:self];
    [self getAllItemsSize];
    [self initialiseAllAttributes];
    [self resetPosition];
}
        ...
        if (min && max) {
            ...
            if (selectedInRange || firstItemInRange || lastItemInRange) {
                ...
                if (!self.getSelectedItemsAfterPanGestureIsFinished && [self.section integerValue] != section) {
                    [self.dataSource sectionForSectionLJIndexTitle:self.indexItems[section] atIndex:section];
                }
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    [self.dataSource sectionForSectionLJIndexTitle:self.indexItems[[self.section integerValue]] atIndex:[self.section integerValue]];
